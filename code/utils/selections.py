...
def model_select(args, model_name):
    if model_name == 'depth_ref':
        from model import splidar_network
        model = splidar_network.Model(args)
        print('Model model loaded from model.py')
        return model
    else:
...
def ref_loss_fun_select(args):
    if args.ref_loss_fun_name == 'MSE':
        return nn.MSELoss(reduction='mean'), nn.L1Loss(reduction='mean')
    elif args.ref_loss_fun_name == 'L1':
        return nn.L1Loss(reduction='mean'), nn.L1Loss(reduction='mean')
    elif args.ref_loss_fun_name == 'char':
        return CharbonnierLoss(), nn.L1Loss(reduction='mean')
    elif args.ref_loss_fun_name == 'L1_grad':
        return L1_grad_loss(args.device), nn.L1Loss(reduction='mean')
    elif args.ref_loss_fun_name == 'L2_grad':
        return L2_grad_loss(args.device), nn.MSELoss(reduction='mean')
    elif args.ref_loss_fun_name == 'L1_grad_pl':
...
def depth_loss_fun_select(args):
    if args.depth_loss_fun_name == 'MSE':
        return nn.MSELoss(reduction='mean'), nn.L1Loss(reduction='mean')
    elif args.depth_loss_fun_name == 'L1':
        return nn.L1Loss(reduction='mean'), nn.L1Loss(reduction='mean')
    elif args.depth_loss_fun_name == 'char':
        return CharbonnierLoss(), nn.L1Loss(reduction='mean')
    elif args.depth_loss_fun_name == 'L1_grad':
        return L1_grad_loss(args.device), nn.L1Loss(reduction='mean')
    elif args.depth_loss_fun_name == 'L2_grad':
        return L2_grad_loss(args.device), nn.MSELoss(reduction='mean')
    elif args.depth_loss_fun_name == 'L1_grad_pl':
        print('Cost function includes perceptual loss...')
        return L1_grad_perceptual_loss(args.device), nn.L1Loss(reduction='mean')
    else:
        print('Invalid loss function, exiting...')
...
class L2_grad_loss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return self.L2loss(x, y) + 0.5 * (self.L1loss(G_x_x, G_y_x) + self.L1loss(G_x_y, G_y_y))
